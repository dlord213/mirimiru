...
export default function ViewPopularReleases() {
  const [isInitialized, setIsInitialized] = useState(false);
  const [IsNSFWVisible, setNSFWVisibility] = useState(false);
  const [currentMangaPage, setCurrentMangaPage] = useState(1);

  const { data: hottestMangaReleases } = useQuery({
    queryKey: ["links_hottestreleases", currentMangaPage],
    queryFn: async () => {
      const { data } = await axios.get(
        `/api/manganato/get-hottest-manga?page=${currentMangaPage}`,
      );
          ...
          <div className="grid grid-cols-4 gap-4 rounded-3xl border border-gray-200 bg-gray-50 p-8 shadow-md dark:border-none dark:bg-stone-800">
            {hottestMangaReleases.mangas.map((release: any) => (
              ...
              >
                ...
                />
                <h1 className="text-xl font-medium text-ellipsis dark:text-stone-50">
                  {release.title}
                </h1>
              </NavLink>
            ))}
